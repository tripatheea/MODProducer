...
int main() {
	...
	while (pfcandidates_triggers_file >> pf_run_number >> pf_event_number >> name >> fired >> prescale1 >> prescale2) {
		...
		// Now start writing out the 4-vectors.
		output_file << "  " << std::noshowpos << fired <<  "         " << std::setw(3) << std::setfill('0') << std::noshowpos << prescale1 << std::setw(3) << std::setfill('0') << "          " << std::setw(3) << std::setfill('0') << std::noshowpos << prescale2 << "        " << name << endl;
