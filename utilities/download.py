...
for f1 in os.listdir(destination_dir):
	deeper_path = destination_dir + f1
	for f2 in os.listdir(deeper_path):	
		if (f2.endswith("root")):
			downloaded_files.append(f2)
...
def download_root_files(list_of_downloads_file, destination_dir):
	...
	for root_link in files_to_download:
		...
		destination_dir_path = destination_dir + "/" + root_link[75:79]
		if (root_file_name not in downloaded_files):
			call(["wget", http_link, "-N", "-P", destination_dir_path])
		i += 1
		print "\n"*5
