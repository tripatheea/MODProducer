...
- Now that we have created a registry for all the AOD files that we want to process, we are ready to run another EDProducer called PFCandidateProducer to convert all the AOD files to MOD (MIT Open Data). We start PFCandidateProducer with the Python file `PFCandidateRun.py`. This Python file takes three arguments: input directory (path to directory which contains all the AOD files), output base path (path to the directory where you want to save the MOD files), registry file path (path, including the filename, to the registry file). Note that to get trigger prescales PFCandidateProducer needs to load globaltags because of which it takes a long time before anything happens. So be patient:

`python ./PFCandidateRun.py ~/opendata/eos/opendata/cms/Run2010B/Jet/AOD/Apr21ReReco-v1/0000/ ~/opendata/eos/opendata/cms/Run2010B/Jet/MOD/Apr21ReReco-v1/0000/ ~/opendata/registry.txt`
